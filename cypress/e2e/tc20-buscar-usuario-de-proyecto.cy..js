...
Desarrollador: Podemos iniciar la próxima semana, prepararé el backlog inicial.`
    ...
    // Hacer clic en el botón de eliminar (icono de papelera)
    cy.get('table')
      .find('button')
      .find('.lucide-trash2')
      .click()
    ...
    // Verificar que el miembro ya no existe en la tabla
    cy.get('table')
      .within(() => {
        cy.contains('td', 'Desarrollador').should('not.exist')
        cy.contains('td', 'Junior').should('not.exist')
      })
  })
})